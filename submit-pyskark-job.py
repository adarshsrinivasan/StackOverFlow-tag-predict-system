...
Usage:
    python submit-pyskark-job --project_id <PROJECT_ID> --region <REGION> \
        --cluster_name <CLUSTER_NAME> --pyspark_file <GCS_JOB_FILE_PATH>
"""
...
def delete_dataproc_cluster(project_id, region, cluster_name):
    # Create the cluster client.
    cluster_client = dataproc_v1.ClusterControllerClient(
        client_options={"api_endpoint": "{}-dataproc.googleapis.com:443".format(region)}
    )
...
def submit_job(project_id, region, cluster_name, gcs_bucket, pyspark_file):
...
if __name__ == "__main__":
    ...
    # create_dataproc_cluster(args.project_id, args.region, args.cluster_name)
    submit_job(args.project_id, args.region, args.cluster_name, args.gcs_bucket, args.pyspark_file)
    # delete_dataproc_cluster(args.project_id, args.region, args.cluster_name)